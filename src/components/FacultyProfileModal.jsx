        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="relative bg-white rounded-3xl shadow-2xl mx-auto w-full max-w-2xl my-8 flex flex-col items-center"
            >
              <button
                onClick={onClose}
                className="absolute top-6 right-6 bg-gray-100 hover:bg-indigo-200 text-gray-800 font-bold rounded-full p-2 shadow transition z-10"
                aria-label="Close"
              >
                <X size={24} />
              </button>
              <div className="flex flex-col items-center pt-10 pb-3 w-full border-b">
                <img
                  src={faculty.photo}
                  alt={faculty.name}
                  className="w-24 h-24 rounded-full object-cover border-4 border-indigo-200 shadow-xl mb-2"
                />
                <h2 className="text-2xl font-bold text-gray-900">{faculty.name}</h2>
                <div className="text-xs text-gray-500 mt-1 mb-1">ID: {faculty.id}</div>
                <div className="text-sm text-indigo-500 font-semibold mb-3">Dept: {faculty.dept || 'Staff'}</div>
              </div>

              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-8 py-5 px-6">
                <div>
                  <div className="mb-4 bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-2xl p-6 shadow-inner text-center">
                    <div className="flex items-center gap-2 justify-center mb-2">
                      <TrendingUp size={22} className="text-pink-400" />
                      <span className="font-semibold text-gray-700">Attendance Percentage</span>
                    </div>
                    <div className="text-5xl font-extrabold text-indigo-600 mb-4 tracking-tight">{attendancePercentage}%</div>
                    <div className="w-full mb-3">
                      <div className="relative w-full h-3 rounded-full bg-gray-200 overflow-hidden">
                        <div
                          className={`absolute top-0 left-0 h-full rounded-full ${
                            attendancePercentage >= 80 ? "bg-green-400" :
                            attendancePercentage >= 60 ? "bg-yellow-400" : "bg-red-400"
                          }`}
                    ...
                    {attendancePercentage < 80 && (
                      <div className="bg-red-100 text-red-700 rounded-lg p-3 text-sm font-medium flex items-center gap-2 mt-3">
                        <AlertCircle size={18} />
                  ...
                  <button
                    onClick={handleMarkAttendanceClick} // ← FIXED: Now opens camera
                    className="w-full mt-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3 rounded-xl font-bold text-lg shadow-sm flex items-center justify-center gap-2 hover:shadow-lg transition"
                  >
                    <Camera size={21} />
                    Mark Attendance
                  </button>
                ...
                <div>
                  <div className="flex gap-2 mb-4 bg-gray-100 p-1 rounded-xl w-full">
                  ...
                  <div className="bg-white rounded-xl p-4 max-h-80 overflow-y-auto border border-gray-200">
